...
router.post('/', async (req, res) => {
    ...
    const newBook = new Book({ title, author, genere, publication_date });
...
router.patch('/:id', getBook, async (req, res) => {
    if (!req.body.title && !req.body.author && !req.body.genere && !req.body.publication_date) {
        res.status(400).json({ message: 'Al menos uno de estos campos debe ser enviado: title, author, genere, publication_date' });
    }
    try {
        const book = res.book;
        book.title = req.body.title || book.title;
